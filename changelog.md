...
- **Strict Bracketed Log Severity Filtering**: Restricted log severity analysis in `genserv.py` and `static/js/genmon.js` to keywords enclosed within `[...]` brackets (e.g. `[WARN]`, `[ERROR]`, `[INFO]`), completely eliminating false-positive warning highlights caused by message content (e.g. 'Genmon System Warning') later in the sentence. Bumped Service Worker cache to `genmon-v10`.
