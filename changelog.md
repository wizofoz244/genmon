...
- **Strict Bracketed Log Severity Filtering**: Restricted log severity analysis in `genserv.py` and `static/js/genmon.js` to keywords enclosed within `[...]` brackets (e.g. `[WARN]`, `[ERROR]`, `[INFO]`), completely eliminating false-positive warning highlights caused by message content (e.g. 'Genmon System Warning') later in the sentence.
- **Favicon Routing & Service Worker Resilience**: Added explicit `/favicon.ico` and `/static/favicon.ico` routes in `genserv.py` and enhanced Service Worker fetch error handling with graceful fallback responses, eliminating console promise conversion errors during service restarts and page reloads. Bumped Service Worker cache to `genmon-v11`.
- **Mobile Web App Meta Tag Standard**: Added `<meta name="mobile-web-app-capable" content="yes">`, clearing Chrome deprecation warnings.
- **Addon Performance & Polling Engine Optimization**:
  - **Backend In-Memory Caching (`genserv.py`)**: Added thread-safe in-memory caching to `services_status_json` (15s TTL) and `script_logs_json` (10s TTL), eliminating heavy repeated `psutil` process scans, Tailscale subprocess execution, and SD card file I/O on every poll. Caches invalidate instantly on service restart or log clear.
  - **Resilient Frontend Polling Engine (`static/js/genmon.js`)**: Increased AJAX timeout from 4s to 10s (`ajaxTimeout: 10000`) for Tailscale Funnel / Wi-Fi tolerance; added in-flight concurrency guards across `fetchStatus`, `fetchPage`, `fetchRegs`, and dashboard tiles to prevent duplicate requests and cascading queue locks; decoupled and staggered auxiliary tile updates (services every 30s at tick 2, script logs every 60s at tick 7); tightened connection health evaluation to require 5 consecutive core telemetry failures before showing the Disconnected banner. Bumped Service Worker cache to `genmon-v12`.
  - **Graceful Startup Retry (`addon/genmaint_sync.py`)**: Added multi-attempt retry loop in `connect_client()` to wait quietly for the core daemon during restarts instead of logging connection errors.
...
- Renamed Unsupported Sensors to Experimental Data
