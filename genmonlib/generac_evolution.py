...
LOG_DEPTH = 50
START_LOG_STARTING_REG = (
    0x012C  # the most current start log entry should be at this register
)
START_LOG_STRIDE = 4
START_LOG_END_REG = (
    START_LOG_STARTING_REG + (START_LOG_STRIDE * LOG_DEPTH)
) - START_LOG_STRIDE
ALARM_LOG_STARTING_REG = (
    0x03E8  # the most current alarm log entry should be at this register
)
ALARM_LOG_STRIDE = 5
ALARM_LOG_END_REG = (
    ALARM_LOG_STARTING_REG + (ALARM_LOG_STRIDE * LOG_DEPTH)
) - ALARM_LOG_STRIDE
SERVICE_LOG_STARTING_REG = (
    0x04E2  # the most current service log entry should be at this register
)
SERVICE_LOG_STRIDE = 4
SERVICE_LOG_END_REG = (
    SERVICE_LOG_STARTING_REG + (SERVICE_LOG_STRIDE * LOG_DEPTH)
) - SERVICE_LOG_STRIDE
...
NEXUS_ALARM_LOG_END_REG = (
    NEXUS_ALARM_LOG_STARTING_REG + (NEXUS_ALARM_LOG_STRIDE * LOG_DEPTH)
) - NEXUS_ALARM_LOG_STRIDE
...
class Evolution(GeneratorController):
    ...
    def InitDevice(self):
        ...
        try:
            ...
            if self.EvolutionController:
                self.ModBus.ProcessTransaction(
                    "%04x" % ALARM_LOG_STARTING_REG, ALARM_LOG_STRIDE
                )
            else:
                self.ModBus.ProcessTransaction(
        ...
        try:
            with self.ExternalDataLock:
                ...
                Tile = MyTile(
                    ...
                    type="batteryvolts",
                    nominal=12,
                    callback=self.GetBatteryVoltage,
                ...
                Tile = MyTile(
                    ...
                    callback=self.GetUtilityVoltage,
                    callbackparameters=(True,),
                )
                self.TileList.append(Tile)
                ...
                if not self.PreNexus and not self.PowerPact:
                    Tile = MyTile(
                        ...
                        type="linevolts",
                        nominal=self.NominalLineVolts,
                        callback=self.GetVoltageOutput,
                        callbackparameters=(True,),
                    )
                    self.TileList.append(Tile)
        ...
        try:
            ...
            if self.NewInstall:
                if not self.ModelIsValid() or self.NominalKW == "Unknown":
                    ReturnStatus, ReturnModel, ReturnKW = self.LookUpSNInfo(
                        SkipKW=(not self.NominalKW == "Unknown"), NoLookUp=NoLookUp
                    )
                    if not ReturnStatus:
                        ...
                        if (
                            ReturnKW != "Unknown" and self.NominalKW == "Unknown"
                        ):  # we found a valid Kw on the lookup
    ...
    def FuelSensorSupported(self):
        ...
        if (
            ...
            and self.UseFuelSensor
            and self.FuelType.lower() == "diesel"
        ):
            return True
        return False
            ...
            for Item in ReturnList:
                ...
                if self.LiquidCooled:
                    try:
                        ...
                        self.LogErrorLine(
                            "Error in GetFuelParamsFromFile (LC): " + str(e1)
                        )
                        continue
                    ...
                    try:
                        ...
                        self.LogErrorLine(
                            "Error in GetFuelParamsFromFile (AC): " + str(e1)
                        )
                        continue
    ...
    def GetLiquidCooledParams(self, ParamGroup, VoltageCode):
        ...
        try:
            ...
            self.LogDebug(
                "Unable to find match for Param Group and Voltage Code: "
                + str(ParamGroup)
                + ", "
                + str(VoltageCode)
            )
    ...
    def GetLiquidCooledModelInfo(self, Request):
        ...
        if self.LiquidCooledParams == None:
            self.LiquidCooledParams = self.GetLiquidCooledParams(
                self.GetParameter("020a", ReturnInt=True),
                self.GetParameter("020b", ReturnInt=True),
            )
    ...
    def GetModelInfo(self, Request):
        ...
        if ModelInfo == UnknownList:
            self.FeedbackPipe.SendFeedback(
                "ModelID",
                Message="Model ID register is unknown: %s" % str(Value),
                FullLogs=True,
            )
    ...
    def LookUpSNInfo(self, SkipKW=False, NoLookUp=False):
        ...
        if not len(SerialNumber) or not len(Controller):
            self.LogError(
                "Error in LookUpSNInfo: bad input, no serial number or controller info not present. Possible issue with serial comms."
            )
            return False, ReturnModel, ReturnKW

        if (
            "none" in SerialNumber.lower() or "unknown" in SerialNumber.lower()
        ):  # serial number is not present due to controller being replaced
            self.LogError(
                "Error in LookUpSNInfo: No valid serial number, controller likely replaced."
            )
            return False, ReturnModel, ReturnKW
    ...
    def DetectController(self, Simulation=False):
        ...
        if self.EvolutionController == None:
            ...
            self.LogError(
                "DetectController auto-detect override (controller). EvolutionController now is %s"
                % str(self.EvolutionController)
            )
            ...
            if counter % 6 == 0:
                for PrimeReg, PrimeInfo in self.PrimeRegisters.items():
                    ...
                    if (
                        ...
                        if self.WaitForExit(
                            "ProcessThread",
                            float(self.ModBus.ModBusPacketTimoutMS / 1000.0),
                        ):  #
                            return
                        self.ModBus.Flush()
                # check for unknown events (i.e. events we are not decoded) and send an email if they occur
    ...
    def UpdateLogRegistersAsMaster(self):
        ...
        for Register in self.LogRange(
            START_LOG_STARTING_REG, LOG_DEPTH, START_LOG_STRIDE
        ):
            RegStr = "%04x" % Register
        ...
        if self.EvolutionController:
            # Service Log
            for Register in self.LogRange(
                SERVICE_LOG_STARTING_REG, LOG_DEPTH, SERVICE_LOG_STRIDE
            ):
                RegStr = "%04x" % Register
            ...
            # Alarm Log
            for Register in self.LogRange(
                ALARM_LOG_STARTING_REG, LOG_DEPTH, ALARM_LOG_STRIDE
            ):
                RegStr = "%04x" % Register
            ...
            # Alarm Log
            for Register in self.LogRange(
                NEXUS_ALARM_LOG_STARTING_REG, LOG_DEPTH, NEXUS_ALARM_LOG_STRIDE
            ):
                RegStr = "%04x" % Register
    ...
    def TestCommand(self, CmdString):
        ...
        try:
            ...
            if len(CmdList) != 2:
                self.LogError(
                    "Validation Error: Error parsing command string in TestCommand (parse): "
                    + CmdString
                )
                return msgbody
            ...
            if not CmdList[0].lower() == "testcommand":
                self.LogError(
                    "Validation Error: Error parsing command string in TestCommand (parse2): "
                    + CmdString
                )
                return msgbody
            ...
            self.LogErrorLine(
                "Validation Error: Error parsing command string in TestCommand: "
                + CmdString
            )
    ...
    def AltSetGeneratorExerciseTime(self, CmdString):
        ...
        try:
            ...
            DayOfWeek = {
                "monday": 0,  # decode for register values with day of week
            ...
            Day, Hour, Minute, ModeStr = self.ParseExerciseStringEx(
                CmdString, DayOfWeek
            )

        except Exception as e1:
            self.LogErrorLine(
                "Validation Error: Error parsing command string in AltSetGeneratorExerciseTime: "
                + CmdString
            )
            self.LogError(str(e1))
            return msgbody
        ...
        if Minute < 0 or Hour < 0 or Day < 0:  # validate settings
            self.LogError(
                "Validation Error: Error parsing command string in AltSetGeneratorExerciseTime (v1): "
                + CmdString
            )
            return msgbody
        ...
        if not ModeStr.lower() in ["weekly"]:
            self.LogError(
                "Validation Error: Error parsing command string in AltSetGeneratorExerciseTime (v2): "
                + CmdString
            )
            return msgbody

        # Get System time and create a new datatime item with the target exercise time
        GeneratorTime = datetime.datetime.strptime(
            self.GetDateTime(), "%A %B %d, %Y %H:%M"
        )
        # fix hours and min in gen time to the requested exercise time
        TargetExerciseTime = GeneratorTime.replace(
            hour=Hour, minute=Minute, day=GeneratorTime.day
        )
    ...
    def SetGeneratorExerciseTime(self, CmdString):
        ...
        try:
            ...
            self.LogErrorLine(
                "Validation Error: Error parsing command string in SetGeneratorExerciseTime: "
                + CmdString
            )
            self.LogError(str(e1))
            return msgbody
        ...
        if Minute < 0 or Hour < 0 or Day < 0:  # validate Settings
            self.LogError(
                "Validation Error: Error parsing command string in SetGeneratorExerciseTime (v1): "
                + CmdString
            )
            return msgbody
        ...
        if not self.bEnhancedExerciseFrequency:
            if ModeStr.lower() in ["biweekly", "monthly"]:
                self.LogError(
                    "Validation Error: Biweekly and Monthly Exercises are not supported. "
                    + CmdString
                )
                return msgbody
    ...
    def ParseExerciseStringEx(self, CmdString, DayDict):
        ...
        try:
            ...
            if "setexercise" not in CmdString.lower():
                self.LogError(
                    "Validation Error: Error parsing command string in ParseExerciseStringEx (setexercise): "
                    + CmdString
                )
            ...
            if len(Items) != 2:
                self.LogError(
                    "Validation Error: Error parsing command string in ParseExerciseStringEx (command): "
                    + CmdString
                )
                return Day, Hour, Minute, ModeStr
            ...
            if len(ParsedItems) < 2 or len(ParsedItems) > 3:
                self.LogError(
                    "Validation Error: Error parsing command string in ParseExerciseStringEx (items): "
                    + CmdString
                )
                return Day, Hour, Minute, ModeStr
            ...
            if ModeStr.lower() not in ["weekly", "biweekly", "monthly"]:
                self.LogError(
                    "Validation Error: Error parsing command string in ParseExerciseStringEx (Mode): "
                    + CmdString
                )
                return Day, Hour, Minute, ModeStr
            ...
            if ModeStr.lower() != "monthly":
                ...
                if Day == -1:
                    self.LogError(
                        "Validation Error: Error parsing command string in ParseExerciseStringEx (day of week): "
                        + CmdString
                    )
                    return -1, -1, -1, ""
            else:
            ...
            self.LogErrorLine(
                "Validation Error: Error parsing command string in ParseExerciseStringEx: "
                + CmdString
            )
            self.LogError(str(e1))
            return -1, -1, -1, ""
        ...
        if not ModeStr.lower() in ["weekly", "biweekly", "monthly"]:
            self.LogError(
                "Validation Error: Error parsing command string in ParseExerciseStringEx (v2): "
                + CmdString
            )
            return -1, -1, -1, ""
        ...
        if Minute < 0 or Hour < 0 or Day < 0:  # validate Settings
            self.LogError(
                "Validation Error: Error parsing command string in ParseExerciseStringEx (v3): "
                + CmdString
            )
            return -1, -1, -1, ""
        ...
        if ModeStr.lower() in ["weekly", "biweekly"]:
            if Minute > 59 or Hour > 23 or Day > 6:  # validate Settings
                self.LogError(
                    "Validation Error: Error parsing command string in ParseExerciseStringEx (v4): "
                    + CmdString
                )
                return -1, -1, -1, ""
        else:
            if Minute > 59 or Hour > 23 or Day > 28:  # validate Settings
                self.LogError(
                    "Validation Error: Error parsing command string in ParseExerciseStringEx (v5): "
                    + CmdString
                )
                return -1, -1, -1, ""
    ...
    def SetGeneratorQuietMode(self, CmdString):
        ...
        try:
            ...
            if len(CmdList) != 2:
                self.LogError(
                    "Validation Error: Error parsing command string in SetGeneratorQuietMode (parse): "
                    + CmdString
                )
                return msgbody
            ...
            if not CmdList[0].lower() == "setquiet":
                self.LogError(
                    "Validation Error: Error parsing command string in SetGeneratorQuietMode (parse2): "
                    + CmdString
                )
                return msgbody
            ...
            if "on" in Mode.lower():
                ...
                self.LogError(
                    "Validation Error: Error parsing command string in SetGeneratorQuietMode (value): "
                    + CmdString
                )
                return msgbody
            ...
            self.LogErrorLine(
                "Validation Error: Error parsing command string in SetGeneratorQuietMode: "
                + CmdString
            )
            self.LogError(str(e1))
            return msgbody
    ...
    def UpdateRegisterList(self, Register, Value, IsString=False, IsFile=False, IsCoil = False, IsInput = False):
        ...
        if len(Register) != 4 or len(Value) < 4:
            self.LogError(
                "Validation Error: Invalid data in UpdateRegisterList: %s %s"
                % (Register, Value)
            )
            return False
    ...
    def DisplayMaintenance(self, DictOut=False, JSONNum=False):
        ...
        try:
            ...
            Maintenance["Maintenance"].append(
                {"Generator Serial Number": self.GetSerialNumber()}
            )
            Maintenance["Maintenance"].append(
            ...
            Maintenance["Maintenance"].append(
                {"Nominal Frequency": str(self.NominalFreq) + " Hz"}
            )
            Maintenance["Maintenance"].append({"Fuel Type": self.FuelType})
            Maintenance["Maintenance"].append(
                {"Generator Phase": self.GetModelInfo("phase")}
            )

            if self.EngineDisplacement != "Unknown":
                Maintenance["Maintenance"].append(
                    {
                        "Engine Displacement": self.UnitsOut(
                            self.EngineDisplacement, type=float, NoString=JSONNum
                        )
                    }
                )
            ...
            if self.EvolutionController and self.Evolution2:
                if self.UseMetric:
                    Value = self.ConvertFahrenheitToCelsius(
                        self.GetParameter("05ed", ReturnInt=True)
                    )
                    Maintenance["Maintenance"].append(
                        {
                            "Ambient Temperature Sensor": self.ValueOut(
                                Value, "C", JSONNum
                            )
                        }
                    )
                    ...
                    Maintenance["Maintenance"].append(
                        {
                            "Ambient Temperature Sensor": self.ValueOut(
                                self.GetParameter("05ed", ReturnInt=True), "F", JSONNum
                            )
                        }
                    )
            ...
            if self.EvolutionController and not self.LiquidCooled:
                ControllerSettings.append(
                    {
                        "Calibrate Current 1": self.ValueOut(
                            self.GetParameter("05f6", ReturnInt=True), "", JSONNum
                        )
                    }
                )
                ControllerSettings.append(
                    {
                        "Calibrate Current 2": self.ValueOut(
                            self.GetParameter("05f7", ReturnInt=True), "", JSONNum
                        )
                    }
                )
            ...
            if not self.PreNexus:
                ControllerSettings.append(
                    {
                        "Calibrate Volts": self.ValueOut(
                            self.GetParameter("0208", ReturnInt=True), "", JSONNum
                        )
                    }
                )

            ControllerSettings.append(
                {"Nominal Line Voltage": str(self.GetModelInfo("nominalvolts")) + " V"}
            )
            ControllerSettings.append(
                {"Rated Max Power": str(self.GetModelInfo("kw")) + " kW"}
            )
            if self.LiquidCooled:

                ControllerSettings.append(
                    {
                        "Param Group": self.ValueOut(
                            self.GetParameter("020a", ReturnInt=True), "", JSONNum
                        )
                    }
                )
                ControllerSettings.append(
                    {
                        "Voltage Code": self.ValueOut(
                            self.GetParameter("020b", ReturnInt=True), "", JSONNum
                        )
                    }
                )
                ...
                try:
                    ...
                    ControllerSettings.append(
                        {"Phase": self.ValueOut(int(phase), "", JSONNum)}
                    )
                except:
                    pass
            ...
            if not self.EvolutionController and self.LiquidCooled:
                ...
                Service.append(
                    {
                        "Air Filter Service Due": self.GetServiceDue("AIR")
                        + " or "
                        + self.GetServiceDueDate("AIR")
                    }
                )
                Service.append(
                    {
                        "Oil Change and Filter Due": self.GetServiceDue("OIL")
                        + " or "
                        + self.GetServiceDueDate("OIL")
                    }
                )
                Service.append(
                    {
                        "Spark Plug Change Due": self.GetServiceDue("SPARK")
                        + " or "
                        + self.GetServiceDueDate("SPARK")
                    }
                )
                ...
                Service.append(
                    {
                        "Oil and Oil Filter Service Due": self.GetServiceDue("OIL")
                        + " or "
                        + self.GetServiceDueDate("OIL")
                    }
                )
                Service.append(
                    {
                        "Spark Plug Service Due": self.GetServiceDue("SPARK")
                        + " or "
                        + self.GetServiceDueDate("SPARK")
                    }
                )
                Service.append(
                    {
                        "Battery Service Due": self.GetServiceDue("BATTERY")
                        + " or "
                        + self.GetServiceDueDate("BATTERY")
                    }
                )
            else:
                # Evolution
                if self.PowerPact:
                    ...
                    Service.append(
                        {
                            "Service A Due": self.GetServiceDue("A")
                            + " or "
                            + self.GetServiceDueDate("A")
                        }
                    )
                    Service.append(
                        {
                            "Service B Due": self.GetServiceDue("B")
                            + " or "
                            + self.GetServiceDueDate("B")
                        }
                    )
                    if not self.LiquidCooled:
                        Service.append(
                            {"Battery Check Due": self.GetServiceDueDate("BATTERY")}
                        )

            Service.append(
                {
                    "Total Run Hours": self.ValueOut(
                        self.GetRunHours(ReturnFloat=True), "h", JSONNum
                    )
                }
            )
    ...
    def DisplayExperimentalSensors(self):
        ...
        if self.EvolutionController and self.LiquidCooled:
            ...
            if len(Value):
                ...
                Celsius = float(
                    (-0.2081 * SensorValue**2) + (10.928 * SensorValue) - 129.02
                )
                ...
                Sensors.append(
                    {
                        "Ambient Temp Thermistor": "Sensor: "
                        + Value
                        + ", "
                        + CStr
                        + "C, "
    ...
    def ParseLogEntry(self, Value, LogBase=None):
        ...
        if self.PreNexus:
            Month, Day, Year, Hour, Min, Seconds, LogCode = self.ParsePreNexusLog(
                Value, LogBase=LogBase
            )
            if Month == None:
                return ""
        else:
        ...
        if isinstance(Decoder, str):
            self.LogError(
                "Error in ParseLog: Invalid Base Register %X", ALARM_LOG_STARTING_REG
            )
            return Decoder
        ...
        if (
            LogBase == ALARM_LOG_STARTING_REG
            and "unknown" in LogStr.lower()
            and self.EvolutionController
            and len(Value) > 16
        ):
    ...
    def GetTransferStatus(self):
        ...
        return self.GetParameterBit(
            "0053", 0x01, OnLabel="Generator", OffLabel="Utility"
        )
    ...
    def GetAlarmState(self):
        ...
        if "unknown" in outString.lower():
            self.FeedbackPipe.SendFeedback(
                "Alarm", Always=True, Message="Reg 0001 = %08x" % RegVal, FullLogs=True
            )
    ...
    def GetSensorInputs(self):
        ...
        DealerInputs_Evo_AC = {
            0x0001: [
                True,
                "Manual",
            ],  # Bits 0 and 1 are only momentary (i.e. only set if the button is being pushed)
            0x0002: [
                True,
                "Auto",
            ],  # Bits 0 and 1 are only set in the controller Dealer Test Menu
        ...
        DealerInputs_Evo_LC = {
            ...
            0x0200: [True, "Fuel Pressure / Level Low"],
        }  # Gasoline / Diesel

        if not "diesel" in self.FuelType.lower():
            DealerInputs_Evo_LC[0x0200] = [False, "Fuel below 5 inch"]
            DealerInputs_Evo_LC[0x0020] = [
                False,
                "",
            ]  # Ruptured Basin is not valid for non diesel systems
    ...
    def GetDigitalOutputs(self):
        ...
        # Liquid cooled
        DigitalOutputs_LC = {
            0x01: [True, "Transfer Switch Activated"],
    ...
    def GetDateTime(self):
        ...
        FullDate = (
            self.DaysOfWeek.get(int(DayOfWeek, 16), "INVALID")
            + " "
            + self.MonthsOfYear.get(int(Month, 16), "INVALID")
        )
        FullDate += (
            " " + str(int(DayOfMonth, 16)) + ", " + "20" + str(int(Year, 16)) + " "
        )
    ...
    def GetCurrentOutput(self, ReturnFloat=False, force_sensor = False, leg = None):
        ...
        try:
            ...
            if (
                Voltage > 100
            ):  # only bounds check if the voltage is over 100V to give things a chance to stabalize
                if (
                    CurrentOutput
                    > ((float(self.NominalKW) * 1000) / self.NominalLineVolts) + 2
                    or CurrentOutput < 0
                ):
                    # if we are here, then the current is out of range.
                    if (
                        not self.EvolutionController
                        and not self.LiquidCooled
                        and not BaseStatus == "EXERCISING"
                    ):
                        msg = (
                            "Current Calculation: %f, CurrentFloat: %f, Divisor: %f, Offset %f, Debug: %s"
                            % (
                                CurrentOutput,
                                CurrentFloat,
                                Divisor,
                        ...
                        self.FeedbackPipe.SendFeedback(
                            "Current Calculation out of range",
                            Message=msg,
                            FullLogs=True,
                        )
                    if CurrentOutput < 0:
    ...
    def GetPowerOutput(self, ReturnFloat=False):
        ...
        if (
            not len(EngineState)
            or "stopped" in EngineState.lower()
        ...
        try:
            ...
            if not Calculate:
                if (
                    ...
                    # Nexus Liquid Cooled
                    FloatTemp = self.GetParameter(
                        "0008", ReturnFloat=True, Divider=1.0, Label="Hz"
                    )
                    # this is optional
                    ...
                    Divisor = 1.0
                    return self.GetParameter(
                        "0008", ReturnFloat=ReturnFloat, Divider=Divisor, Label="Hz"
                    )
    ...
    def GetPickUpVoltage(self, ReturnInt=False):
        ...
        if (self.EvolutionController and self.LiquidCooled) or (
            self.EvolutionController and self.Evolution2
        ):
    ...
    def GetBatteryVoltage(self, ReturnFloat=False):
        ...
        return self.GetParameter(
            "000a", Label="V", ReturnFloat=ReturnFloat, Divider=10.0
        )
    ...
    def GetBatteryStatus(self):
        ...
        # get Battery Charging Voltage
        return self.GetParameterBit(
            "0053", 0x10, OnLabel="Charging", OffLabel="Not Charging"
        )
    ...
    def GetServiceDue(self, serviceType="A", NoUnits=False):
        ...
        ServiceTypeLookup_Evo = {"A": "001a", "B": "001e"}
        ...
        ServiceTypeLookup_Nexus_LC = {"OIL": "001a", "SPARK": "001e", "AIR": "001c"}
        if self.EvolutionController:
    ...
    def GetServiceDueDate(self, serviceType="A"):
        ...
        ServiceTypeLookup_EvoAC = {"A": "001b", "B": "001f", "BATTERY": "0022"}
        ...
        try:
            ...
            if (
                self.GetDeltaTimeMinutes(
                    datetime.datetime.now() - self.CheckFirmwareVerionsTime
                )
                < 60
            ):  # check every hour
                return
            ...
            if self.SavedFirmwareVersion == None:
                ...
                if FWVersionString != self.SavedFirmwareVersion:
                    ...
                    msgbody = (
                        "NOTE: This message is a notice that the version of the generator controller firmware has changed from %s to %s. \n"
                        % (self.SavedFirmwareVersion, FWVersionString)
                    )
                    self.LogError(
            ...
            Outage["Outage"].append(
                {
                    "Utility Threshold Voltage": self.ValueOut(
                        self.GetThresholdVoltage(ReturnInt=True), "V", JSONNum
                    )
                }
            )
                ...
                Outage["Outage"].append(
                    {
                        "Utility Pickup Voltage": self.ValueOut(
                            self.GetPickUpVoltage(ReturnInt=True), "V", JSONNum
                        )
                    }
                )
    ...
    def DisplayStatus(self, DictOut=False, JSONNum=False, Reg0001Value=None):
        ...
        try:
            ...
            Status["Status"].append(
                {"Last Log Entries": self.DisplayLogs(AllLogs=False, DictOut=True)}
            )
            Status["Status"].append({"Time": Time})

            Engine.append(
                {"Switch State": self.GetSwitchState(Reg0001Value=Reg0001Value)}
            )
            Engine.append(
            ...
            Engine.append(
                {
                    "Battery Voltage": self.ValueOut(
                        self.GetBatteryVoltage(ReturnFloat=True), "V", JSONNum
                    )
                }
            )
            if self.EvolutionController and self.LiquidCooled:
            ...
            Engine.append(
                {
                    "Frequency": self.ValueOut(
                        self.GetFrequency(ReturnFloat=True), "Hz", JSONNum
                    )
                }
            )
            if not self.PreNexus and not self.PowerPact:
                Engine.append(
                    {
                        "Output Voltage": self.ValueOut(
                            self.GetVoltageOutput(ReturnInt=True), "V", JSONNum
                        )
                    }
                )
            if self.PowerZone200:
            ...
            Engine.append(
                {
                    "Active Rotor Poles (Calculated)": self.ValueOut(
                        self.GetActiveRotorPoles(ReturnInt=True), "", JSONNum
                    )
                }
            )
            ...
            Line.append(
                {
                    "Utility Max Voltage": self.ValueOut(
                        self.UtilityVoltsMax, "V", JSONNum
                    )
                }
            )
            Line.append(
                {
                    "Utility Min Voltage": self.ValueOut(
                        self.UtilityVoltsMin, "V", JSONNum
                    )
                }
            )
            ...
            if self.EvolutionController and self.LiquidCooled:
                ...
                Line.append(
                    {
                        "Set Output Voltage": self.ValueOut(
                            self.GetSetOutputVoltage(ReturnInt=True), "V", JSONNum
                        )
                    }
                )
        ...
        try:
            ...
            if self.config != None:
                ...
                if self.config.HasOption("evolutioncontroller"):
                    self.EvolutionController = self.config.ReadValue(
                        "evolutioncontroller", return_type=bool, default=False
                    )
                if self.config.HasOption("liquidcooled"):
                    self.LiquidCooled = self.config.ReadValue(
                ...
                self.bEnhancedExerciseFrequency = self.config.ReadValue(
                    "enhancedexercise", return_type=bool, default=False
                )
                self.CurrentDivider = self.config.ReadValue(
                    "currentdivider", return_type=float, default=None, NoLog=True
                )
                ...
                self.SerialNumberReplacement = self.config.ReadValue(
                    "serialnumberifmissing", default=None
                )
                if self.SerialNumberReplacement != None and len(
                    self.SerialNumberReplacement
                ):
                    if (
                        self.SerialNumberReplacement.isdigit()
                        and len(self.SerialNumberReplacement) == 10
                    ):
                        self.LogError(
                        ...
                        self.LogError(
                            "Override Serial Number: bad format: "
                            + "<"
                            + self.SerialNumberReplacement
                            + ">"
                        )
                ...
                self.AdditionalRunHours = self.config.ReadValue(
                    "additionalrunhours", return_type=float, default=0.0, NoLog=True
                )
                self.UseNominalLineVoltsFromConfig = self.config.ReadValue(
                    "usenominallinevolts", return_type=bool, default=False
                )
            ...
            self.LogErrorLine(
                "Missing config file or config file entries (evo/nexus): " + str(e1)
            )
            return False
